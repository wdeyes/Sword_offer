...
void core( BinaryTreeNode* pNode, unsigned int k, int &count, BinaryTreeNode* &pRet){
	if(pNode==nullptr )
		return ;

	core(pNode->m_pLeft, k, count, pRet);

	++count;
	if( count==k )
		pRet = pNode;

	core(pNode->m_pRight, k, count, pRet);
}
